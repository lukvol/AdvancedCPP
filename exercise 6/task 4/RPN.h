...
private:
	...
	T division();
public:
	RPN(std::string filename) : stack(filename) { }
	void start();
};
...
template<>
std::complex<double> RPN<std::complex<double>>::division() {
	std::complex<double> divisor = pop_element();
	std::complex<double> divident = pop_element();
...
template<>
void RPN<std::complex<double>>::print_minimum() {
	std::complex<double> min = stack[0];
	std::for_each(stack.begin(), stack.end(), [&min](std::complex<double> &x) { 
		if (x.real() < min.real()) {
			min = x;
		} else if (x.real() == min.real()) {
			if (x.imag() < min.imag()) {
				min = x;
			}
		}
...
template<typename T>
void RPN<T>::print_minimum() {
	T min = stack[0];
	std::for_each(stack.begin(), stack.end(), [&min](T &x) { 
		if (x < min) {
			min = x;
		}
	});
...
void RPN<T>::parse_input(char command, std::istringstream &fst) {
	switch (command) {
		...
		case 'n': {
			...
			//i.e., if n (1,2) is the input, the pointer, after reading it is at position 7 instead of -1
